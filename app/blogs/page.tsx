...
const BlogCard = ({ post }: { post: (typeof allPosts)[number] }) => (
  <article className="overflow-hidden rounded-xl bg-[#f7f7f7] shadow-sm">
    <div className="relative min-h-[220px] overflow-hidden bg-black sm:min-h-[245px]">
      <Image
        src={post.image}
        alt={post.title}
        fill
    ...
    <div className="p-5 sm:p-6">
      <h3 className="text-lg font-black leading-snug text-[#242a31] sm:text-xl">{post.title}</h3>
      <p className="mt-3 text-sm font-medium leading-6 text-black/75 sm:text-[15px]">{post.excerpt}</p>
      <div className="mt-6 flex items-center justify-between gap-4">
        <time className="text-xs font-semibold uppercase tracking-wide text-black/60">{post.date}</time>
        <Link
          href={post.href}
          className="inline-flex min-h-10 items-center justify-center rounded-md bg-[#f4070b] px-5 text-sm font-bold text-white transition-colors hover:bg-red-700"
        >
          Read More
        </Link>
      </div>
...
const BlogsPage = () => {
  ...
  return (
    <main className="bg-white text-black">
      <section className="relative min-h-[560px] overflow-hidden bg-black px-4 text-white sm:px-6 lg:px-8">
        <Image src="/b18.png" alt="TAYLORMOT car repair blog" fill priority sizes="100vw" className="object-cover opacity-45" />
        <div className="absolute inset-0 bg-black/55" />
        <div className="relative z-10 mx-auto flex min-h-[560px] max-w-7xl flex-col justify-center">
          <h1 className="max-w-4xl text-3xl font-black uppercase leading-tight sm:text-4xl lg:text-5xl">
            Car Repair & Maintenance Blog
          </h1>
          <p className="mt-5 text-sm font-bold sm:text-base">
            <Link href="/" className="hover:text-[#f4070b]">Home</Link> &raquo; Blog | TAYLORMOT
          </p>
          <div className="mt-10 flex flex-wrap gap-4">
            <a
              href="tel:+441784240000"
              className="inline-flex min-h-12 min-w-44 items-center justify-center rounded-lg bg-white px-7 text-base font-black text-black shadow-lg transition-colors hover:bg-gray-100"
            >
              CALL NOW
            </a>
            <Link
              href="/appointment"
              className="inline-flex min-h-12 min-w-44 items-center justify-center rounded-lg bg-[#f4070b] px-7 text-base font-black text-white shadow-lg shadow-red-900/30 transition-colors hover:bg-red-700"
            >
              BOOK NOW
            </Link>
          </div>
      ...
      <section className="px-4 py-16 text-center sm:px-6 lg:px-8">
        <div className="mx-auto max-w-5xl">
          <p className="text-sm font-bold uppercase tracking-[0.16em] text-[#161b22]">Advice</p>
          <h2 className="mt-4 text-3xl font-black leading-tight text-[#f4070b] sm:text-4xl">Car advice and news</h2>
          <p className="mx-auto mt-7 max-w-3xl text-base font-medium leading-7 text-black/75 sm:text-lg">
          ...
          <form className="mx-auto mt-9 max-w-3xl" onSubmit={(event) => event.preventDefault()}>
            <label className="relative block">
              ...
              <input
                ...
                placeholder="Search..."
                className="h-14 w-full rounded-lg border border-[#f4070b] px-5 pr-14 text-base font-medium outline-none transition-shadow focus:shadow-[0_0_0_4px_rgba(244,7,11,0.12)]"
              />
              <Search className="absolute right-5 top-1/2 h-6 w-6 -translate-y-1/2 text-[#f4070b]" strokeWidth={3} />
            </label>
            <p className="mt-4 text-left text-sm font-medium text-black/65">Type above to filter guides.</p>
          </form>
        </div>
      </section>
        ...
        <section className="px-4 pb-20 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-7xl">
            <div className="mb-8 flex items-center gap-4">
              <h2 className="text-2xl font-black uppercase">Search Results</h2>
            ...
            <div className="mx-auto max-w-7xl">
              <div className="mb-8 flex items-center justify-between gap-5">
                <div className="flex items-center gap-4">
                  <h2 className="text-2xl font-black uppercase leading-tight">{section.title}</h2>
                  <span className="hidden h-1 w-14 bg-[#d6111b] sm:block" />
                </div>
                <Link href="#filter" className="text-sm font-bold uppercase tracking-wide text-[#f4070b] hover:text-red-700">
                  More Guides
                </Link>
              </div>
